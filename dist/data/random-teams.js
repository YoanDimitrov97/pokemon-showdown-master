...
class RandomTeams {
  ...
  getDoublesItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
    ...
    if (species.id === "sylveon")
      return "Pixie Plate";
    if ((offensiveRole || role === "Tera Blast user" && species.baseStats.spe >= 80 && !moves.has("trickroom")) && !moves.has("fakeout") && !moves.has("incinerate") && (!moves.has("uturn") || types.includes("Bug") || species.baseStats.atk >= 120 || ability === "Libero") && (!moves.has("icywind") || species.id === "ironbundle")) {
