...
class RandomGen7Teams extends import_random_teams.RandomGen8Teams {
  constructor(format, prng) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Flying: (movePool, moves, abilities, types, counter, species) => !counter.get("Flying") && ["aerodactylmega", "charizardmegay", "mantine"].every((m) => species.id !== m) && !movePool.includes("hiddenpowerflying"),
  ...
  getItem(ability, types, moves, counter, teamDetails, species, isLead, preferredType, role) {
    ...
    if (moves.has("pursuit") && moves.has("suckerpunch") && counter.get("Dark") && !PRIORITY_POKEMON.includes(species.id))
    ...
    if (ability === "Rough Skin" || species.id !== "hooh" && ability === "Regenerator" && species.baseStats.hp + species.baseStats.def >= 180 && this.randomChance(1, 2))
      return "Rocky Helmet";
    if (["protect", "spikyshield", "substitute"].some((m) => moves.has(m)))
      return "Leftovers";
