...
const Formats = [
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (natdex && item.id !== "ultranecroziumz")
          continue;
        const species = this.dex.species.get(set.species);
        if (species.isNonstandard && !this.ruleTable.has(
    ...
    onSwitchIn(pokemon) {
      ...
      if (originalFormeSecies.exists && pokemon.m.originalSpecies !== originalFormeSecies.baseSpecies) {
        this.add(
          "-start",
          pokemon,
          originalFormeSecies.requiredItem || originalFormeSecies.requiredMove,
          "[silent]"
        );
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (!pokemonWithAbility)
        return [
          `${ability.name} is not available on a legal Pok\xE9mon.`
        ];
  ...
  },
  {
    ...
    onValidateSet(set) {
      const species = this.dex.species.get(set.species);
      const unSeenAbilities = Object.keys(species.abilities).filter(
        (key) => key !== "S" && (key !== "H" || !species.unreleasedHidden)
      ).map((key) => species.abilities[key]).filter((ability) => ability !== set.ability);
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        ...
        pokemon.m.innates = Object.keys(pokemon.species.abilities).filter(
          (key) => key !== "S" && (key !== "H" || !pokemon.species.unreleasedHidden)
        ).map(
          (key) => this.toID(
            pokemon.species.abilities[key]
          )
    ...
    onSwitchOut(pokemon) {
      for (const innate of Object.keys(pokemon.volatiles).filter(
        (i) => i.startsWith("ability:")
      )) {
    ...
    battle: {
      tiebreak() {
        ...
        const hpPercentage = tiedSides.map(
          (side) => side.pokemon.map((pokemon) => pokemon.hp / pokemon.maxhp).reduce((a, b) => a + b) * 100 / 6
        );
        this.add(
          "-message",
          tiedSides.map(
            (side, i) => `${side.name}: ${Math.round(
              hpPercentage[i]
            )}% total HP left`
          ).join("; ")
        );
        ...
        const hpTotal = tiedSides.map(
          (side) => side.pokemon.map((pokemon) => pokemon.hp).reduce((a, b) => a + b)
        );
        this.add(
          "-message",
          tiedSides.map(
      ...
      for (const trademark in teamHas.trademarks) {
        if (teamHas.trademarks[trademark] > 1) {
          problems.push(
            ...
            `(You have ${teamHas.trademarks[trademark]} Pok\xE9mon with ${trademark} as a Trademark.)`
          );
        }
      }
      return problems;
    ...
    onSwitchIn(pokemon) {
      ...
      for (const i in pokemon.species.baseStats) {
        ...
        this.add(
          "-start",
          pokemon,
          `${pokemon.species.baseStats[i]}${i === "spa" ? "spc" : i}`,
          "[silent]"
        );
      }
    }
    ...
    onValidateSet(set) {
      ...
      if (set.moves.length !== 1 || this.dex.moves.get(set.moves[0]).id !== "metronome") {
        return [
          ...
          `(Pok\xE9mon can only have one Metronome in their moveset)`
        ];
      }
    }
  },
  ...
  },
  {
    name: "[Gen 2] 1v1",
    threads: [
      `&bullet; <a href="https://www.smogon.com/forums/posts/8031463/">GSC 1v1</a>`
    ],
    mod: "gen2",
    searchShow: false,
    ruleset: [
